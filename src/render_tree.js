...
Markdown.toHTMLTree = function toHTMLTree(input, dialect, options) {
  ...
  const attrs = extract_attr(input);
  let refs = {};
  if (attrs && attrs.references) refs = attrs.references;
  const html = convert_tree_to_html(input, refs, options);
  merge_text_nodes(html);
  return html;
};
Markdown.toHTML = function toHTML(source, dialect, options) {
  const input = this.toHTMLTree(source, dialect, options);
...
function render_tree(jsonml) {
  ...
  for (const a in attributes) {
    const escaped = escapeHTML(attributes[a]);
